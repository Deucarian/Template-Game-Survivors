...
- PlayMode coverage checks waystone discovery, Waystone Chain, roaming cache special-drop, ambush, and ambush-clear payoff beats using deterministic discovery/cache cadence tuning.
- PlayMode coverage checks the controller starts in Human Playtest, the imported scene starts by path and advances into horde spawning plus a three-choice XP draft, the first XP pickup can be collected within seconds, the first draft can open within one minute, major-threat warnings appear before timed elites/miniboss/boss, reward choices wait for the player by default, and Debug Fast only appears through an explicit profile switch.
- Combat, pickup, and reward feedback coverage checks enemy hit flashes, short-lived enemy death bursts, magnet recall feedback, evolution-triggered XP recall, normal level-up Level Pulse damage, full-weapon Arsenal Surge momentum, full-passive Harmony Surge momentum, Waystone Chain momentum, boss relic Relic Surge momentum, endless major-threat Endless Surge rewards, low-health Clutch Pulse safety and enemy damage, multi-evolution Legend Surge momentum, Gem Rush pickup-cluster stat boosts, XP attraction feedback, pulsing recalled gem presentation, rarity-card presentation, and selected reward banners.
