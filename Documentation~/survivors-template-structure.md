...
The editor provider maps one existing tutorial step to one canonical record by stable authored `id`. Its only writable collection is direct `tutorialSteps[*].lines` in a project-owned imported Basic or Neon theme source. The shared Game Content Authoring workbench supplies generic ordered operations; this template owns direct-array lossless serialization, the 1-3 nonblank-line rule derived from the fixed tutorial panel, strict proposed-pack validation, atomic commit/recovery, and runtime consumption. JSON remains the sole source of truth and no ScriptableObject mirror or tutorial-step CRUD path exists.
