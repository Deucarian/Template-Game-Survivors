...
The sample bootstrap binds `Content/DefaultEnemies/enemies.json`, `Content/DefaultRunFlow/run-flow.json`, and `Content/DefaultRewards/rewards.json` into the runtime controller before opening the run-mode selector. Asset-flip authors can change boss/elite IDs, lifecycle flags, life-bar flags, run timing, draft pacing, rarity tables, and major reward definitions in those JSON files and then rerun content validation without editing controller code.
