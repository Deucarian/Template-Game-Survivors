...
- authored weapon, upgrade, relic, class, progression, enemy, run-flow, reward, UI theme, audio palette, and tutorial-copy binding for the playable sample, with fallback defaults reserved for unbound/debug hosts
...
- authored weapons, projectiles, run-upgrade definitions, explicit effect amounts, rank metadata, draft-card metadata, progression-derived upgrade gates, evolutions, pickup/magnet passives, relics, classes, one-ability starter loadouts, and progression tracks
