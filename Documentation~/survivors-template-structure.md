...
- clean player HUD, F1-gated debug overlay, full-screen draft-card overlay, and pause/build menu tabs for Current Build, Stats, Run Info, and Controls
...
- run-mode display names and common draft/build/debug button copy
...
The sample bootstrap binds `Content/DefaultEnemies/enemies.json`, `Content/DefaultRunFlow/run-flow.json`, `Content/DefaultRewards/rewards.json`, and `Content/DefaultUiTheme/ui-theme.json` into the runtime controller before opening the run-mode selector. Asset-flip authors can change boss/elite IDs, lifecycle flags, life-bar flags, run timing, draft pacing, rarity tables, major reward definitions, draft-card labels, rarity presentation tokens, placeholder icon IDs, and basic UI copy in those JSON files and then rerun content validation without editing controller code.
