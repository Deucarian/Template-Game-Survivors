...
The sample bootstrap strictly binds `Content/DefaultWeapons/weapons.json`, `Content/DefaultUpgrades/upgrades.json`, `Content/DefaultRelics/relics.json`, `Content/DefaultClasses/classes.json`, `Content/DefaultProgression/progression.json`, `Content/DefaultEnemies/enemies.json`, `Content/DefaultRunFlow/run-flow.json`, `Content/DefaultRewards/rewards.json`, `Content/DefaultPickups/pickups.json`, `Content/DefaultUiTheme/ui-theme.json`, and `Content/NeonArcanaUiTheme/ui-theme.json` into the runtime controller before opening the run-mode selector. Asset-flip authors can change weapon roster/stats, starting ability by class, weapon unlocks, weapon upgrade paths, evolutions, passives, pickup/magnet upgrades, relics, classes, progression tracks and class gates, boss/elite IDs, lifecycle/life-bar flags, run timing, shared/profile tuning, draft pacing, rarity tables, reward definitions, draft-card labels, presentation tokens, tutorial/result copy, HUD accent, and audio events in those JSON files and then rerun content validation without editing controller code.
