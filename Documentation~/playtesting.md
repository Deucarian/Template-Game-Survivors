...
To verify safe tutorial authoring, open Game Content Authoring on an imported Basic or Neon pack, select a Tutorial record, and edit `Lines`. Confirm the workbench permits ordered Add/Remove/Move/Replace only within one to three nonblank rows, leaves the stable step ID read-only, previews the complete strict pack, and warns that sample refresh may conflict. After Commit, launch that pack's scene, choose Standard or Sprint, reopen the matching tutorial step, and confirm the authored text/order. Use explicit Rollback for the proof and verify the imported JSON returns to its exact original hash.

To verify safe Upgrade Effects authoring, select `Keen Edge`, `Soulflare Glyph`, `Astral Convergence`, or an Evolution with a direct `effects` array in the imported Basic or Neon pack. Open `Effects`, change a finite non-zero amount, move a row, and exercise Add then Remove with explicit `effect`, `target`, and `amount` values. Confirm the parent Upgrade ID stays read-only, no child record appears in All Content, unsupported token text cannot be entered, Preview validates the complete selected pack, and no source bytes change before Commit. After Commit, start Standard or Sprint for that same pack and confirm the parent Upgrade applies the authored effect. Use explicit Rollback and verify both the edited source hash and the other pack's hash remain exact. A Package Installer refresh can replace imported-sample edits, so review or commit intentional project-owned changes before refreshing the sample.
...
2. Open Deucarian Control Center > Developer > Survivors Runtime Debugger.
