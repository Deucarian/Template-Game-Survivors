...
| Simulation phases and actor iteration | `SurvivorsRunSimulation / SurvivorsActorSimulation` | Clamped phase clock and explicit ordered progress/world commands; reverse iteration and pruning over the original actor lists |
...
| Evolution announcement history | `SurvivorsEvolutionAnnouncements` | Ordinal goal/ready announced-ID sets, borrowed build/catalog queries and existing telemetry/presentation callbacks |
...
| Player, debug and timer rendering | `SurvivorsPlayerHudPresenter / SurvivorsDebugHudPresenter / SurvivorsTopTimerHud` | Copied bars/debug values, current authored compact labels and responsive geometry; no gameplay or screen routing |
...
| Class and result definition labels | `SurvivorsProgressionChoiceLabels` | Copied definitions and current unlock/rank observations; no profile commands |
...
| Build and draft diagnostic descriptions | `SurvivorsDebugBuildModel / SurvivorsDebugDraftModel / SurvivorsDebugUpgradeFormatter` | Borrowed catalog/build observations and explicit current-run entry guards |

| Actor membership and area commands | `SurvivorsActorMembership / SurvivorsAreaDamage` | The authoritative three actor lists, release order and live area-target commands over a query snapshot |
...
| Reward title and accent projection | `SurvivorsDraftHeader` | Live draft and theme reads with relic, empty, evolution and rarity precedence |

| Serialized composition and compatibility bindings | `SurvivorsTemplateController and named partial declarations` | Original field initialization order, Unity messages, guarded public forwards and ordered owner setup/cleanup |
...
Ten cases preserve catalog-ID order, current prerequisite reads, rank/choice formatting and conditional build descriptions.
...
Seven cases preserve title mappings, late theme reads, draft replacement during observation and null/empty distinctions.

A source-equivalence audit preserves all 1,620 retained members and all 137 retained field initializers in their original relative order; the existing public facade and six serialized fields remain compatible.

## Component compatibility boundary

The original component is now 396 lines and retains its script GUID, six serialized declarations and all owner-reference field initializers. Eighteen named partial declarations contain compatibility forwards, copied observations, Unity-null guards and explicit domain ports. Their names identify the bound responsibility; they do not introduce independent component instances or a second mutable state store. Every component source file is below 500 lines. The aggregate remains a large migration surface because existing public diagnostics, test entry points and actor callers remain source compatible.

The component still orders run initialization, owner resets, class/profile resolution, world construction and final cleanup. These are composition decisions: gameplay timing, draft eligibility, combat, reward transactions, encounter membership, stat arithmetic and rendering policy live in the corresponding owners. The initial migration removed 37 unused private helpers and aliases. A subsequent aggregate-method review removed 43 redundant private build, draft and reward aliases: 10 had no callers, and the other 33 now have 40 internal invocation sites calling the same owners directly. No public facade member was removed to meet a file limit.

The syntax-aware registry gate counts the entire component across all partial declarations. Its method count is now 637 against the reviewed limit of 644, down from 680 before the alias review. The existing callback aliases remain where they delay owner resolution until invocation; no method-group capture was replaced with eager owner construction. Direct-call rewrites retain the original arguments, which are locals, constants, out declarations or readonly simulation-frame fields. Owner policy implementations, field initializers, serialized declarations and public/explicit port contracts are unchanged by this follow-up.

This completes the targeted controller responsibility inventory. It does not claim that every pre-existing source file in the package meets A9. BasicSurvivorsGame, authored-content/parser files, the weapon archetype runtime and other untouched oversized legacy files remain separate existing debt; the final package inventory records their exact current line counts. No unrelated balance/content extraction is implied by the component migration.

The XP compatibility follow-up keeps the base multiplier, run/persistent bonus and passive-surge bonus as separate ordered terms before the original clamp and rounding. Two focused arithmetic cases cover ordinary authored-sized bonuses and finite cancellation; a pre-summed pair could otherwise grant a different integer XP amount. Existing single-bonus internal callers retain their overload. This correction restores the original calculation and is separate from the PlayMode fixture's controlled frame timing.

The five-minute Sprint pacing fixture controls its clock explicitly: the component is disabled to prevent automatic Update from adding wall-clock simulation, while the test invokes the original LateUpdate camera callback at the existing yield points. A scoped 0.02-second capture interval controls that presentation callback; finally restores it and destroys the test controller even after failure. All seven original numeric bounds remain unchanged. The same isolated fixture passes against the original development package; this is a test isolation correction rather than a balance change.
