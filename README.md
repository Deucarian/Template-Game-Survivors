...
JSON remains the only gameplay source of truth. For project-owned imported copies under `Assets/Samples`, the shared edit workbench can safely patch approved direct scalar fields on weapon, projectile, and enemy records, one canonical evolution prerequisite, a tutorial step's ordered `Lines`, and an Upgrade's existing direct `Effects` rows. Effects are anonymous values embedded in their parent Upgrade: each row exposes closed `effect` and `target` token selectors plus a finite non-zero `amount`, with explicit Add, Remove, Move, field replacement, Restore Original Order, Undo, and Redo. Adding or removing a row never creates or deletes an Upgrade, and no row gains a stable ID or canonical record identity. Tutorial lines retain their 1-3 nonblank-row contract. Stable IDs, archetypes/roles, every other reference or collection, unsupported/nested row structures, top-level record CRUD, and pack cloning remain read-only. The backend replaces only selected scalar tokens or the selected direct `lines`/`effects` array span, preserves unmapped scalar row data and every byte outside that span, validates the complete strict pack before save, refuses stale sources, writes atomically, and retains exact recovery bytes under `Library/Deucarian/GameContentAuthoring/Recovery/Survivors`. It never creates a ScriptableObject mirror or round-trips a full JSON DTO. See `Documentation~/game-content-authoring.md` for the exact fields, imported-copy warning, cancellation, rollback, and recovery behavior.
