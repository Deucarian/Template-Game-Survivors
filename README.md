...
Weapons auto-fire toward nearby targets. XP gems pull toward the player when close, pulse as they are attracted, vital shards restore health when wounded, and magnet pickups or the debug recall make distant XP gems spin and surge inward with stronger pickup feedback.
...
- Run tuning, pacing profiles, horde pack sizes, health pickup amount, endless major-threat intervals, reward timeout, draft rarity weights, draft rerolls, draft banishes, and skip rewards: start with `CreateDefaultTuning` and `CreateTuning(SurvivorsPacingProfile)` before changing controller internals.
