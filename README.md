...
Playable Unity template package for a Survivors-style horde roguelite loop. The sample boots into a top-down arena with radial enemy spawning, auto-attacks, XP gems, rarity-tinted reward cards, level-up choices, readable damage feedback, enemy hit flashes, death bursts, ranged enemy shot cues, major-reward cache beacons, reward-pick banners, low-health warnings, major-threat telegraphs, miniboss and boss pressure, elite upgrade rewards, boss evolution rewards, relic rewards, class unlocks, persistent meta progression, victory, defeat, and restart flow.
...
- Run tuning, pacing profiles, horde pack sizes, reward timeout, draft rarity weights, draft rerolls, draft banishes, and skip rewards: start with `CreateDefaultTuning` and `CreateTuning(SurvivorsPacingProfile)` before changing controller internals.
...
- 30+ meaningful upgrade choices with visible behavior changes, status/sustain hooks, passive requirements, Epic spikes, and eight legendary evolutions
...
- Combat feedback is hard to read: confirm the imported sample is current, then damage enemies or take a hit; enemies should flash on hit, deaths should leave a short burst, ranged attackers should draw a quick hostile shot cue, major reward kills should pop a colored cache beacon, resolved damage should appear as short-lived numbers, and low health should pulse a red screen-edge warning.
