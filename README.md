...
Playable Unity template package for a Survivors-style horde roguelite loop. The sample boots into a top-down arena with radial enemy spawning, readable horde crowd spacing, timed horde-rush pressure beats with breaker-pulse clear rewards, auto-attacks, XP gems, a short Arc Step dash for emergency spacing, rarity-tinted reward cards, level-up choices, readable damage feedback, enemy hit flashes, death bursts, dodgeable ranged enemy shot cues with small dodge XP rewards, streak reward banners, temporary streak surges, evolution-ready banners, weapon-evolution surge payoffs, Rift Caller support-call banners, splitter fragment banners, major-threat health bars, low-health major-threat support call-ins, ground-marked major-threat slam attacks, major-reward cache beacons, cache-pull pickup bursts, reward-pick banners, low-health warnings, major-threat telegraphs, miniboss and boss pressure, elite upgrade rewards, boss evolution rewards, relic rewards, class unlocks, persistent meta progression, result-screen meta purchases, result-screen class selection, victory/defeat run summaries, endless continuation, and restart flow.
...
4. Press Play.
5. Move through the horde, collect XP gems, choose level-up options, take the elite reward after the miniboss, then build toward a boss evolution reward before defeating the final boss; if no evolution is ready, the boss falls back to a stronger reward draft before victory. Spend earned shards on affordable meta upgrades from the result screen, pick any unlocked next-run class, then continue into endless escalation or restart.
...
The scene contains a tiny bootstrap object. At runtime it creates the arena, player, enemy/pickup/projectile pools, camera, run timer, HUD, draft UI, relic UI, meta profile service, result summary state, endless continuation, and restart flow.
...
Default play uses `SurvivorsPacingProfile.HumanPlaytest`: `Time.timeScale` is reset to `1`, opening enemy spawns use two-enemy packs on a `1.15` second interval, pack size grows with escalation up to `5`, the opening max-alive cap is `34`, basic enemy speed is `1.35` versus player speed `5.4`, nearby enemies nudge apart so packs stay readable while still pressing inward, reward auto-pick is off, horde-rush events warn and then spawn a mixed enemy ring between major threats, clearing the ring drops a bonus cache and fires a breaker pulse into nearby regular enemies, Rift Callers enter the regular horde around `155` seconds and periodically call nearby support enemies, active elites/minibosses/bosses show a prioritized health bar, major enemies show a center-screen warning before they arrive, damaged major enemies call in a one-shot support ring near their low-health threshold, and dread elites/minibosses/bosses paint a brief ground slam disc before damaging nearby players. The first elite arrives around `180` seconds, the dread elite arrives around `300` seconds, the miniboss waits until `420` seconds, the final boss appears around `1200` seconds, and survival victory lands at `1800` seconds. After victory, continuing schedules recurring elite, miniboss, and boss pressure using the endless interval tuning on `SurvivorsTemplateTuning`. `Normal`, `DebugFast`, and `Showcase` profiles exist for validation and demos, but must be selected deliberately through the runtime debugger.
...
- Run tuning, pacing profiles, dash distance/cooldown/safety/shove values, horde pack sizes, enemy crowd-spacing radius/strength, ranged-shot dodge XP rewards, timed horde-rush events, clear caches, and breaker-pulse damage/radius, roaming cache cadence/specials/ambushes/Wayfinder Surge rewards, waystone discovery rewards/focus/ambushes, regular summoner support cadence/counts/extra alive allowance, evolution-surge damage/radius, major-threat support call-in thresholds/counts, major-threat slam interval/telegraph/radius/damage, kill-streak reward/surge pacing, pickup heal/currency amounts, endless major-threat intervals, reward timeout, draft rarity weights, draft rerolls, draft banishes, and skip rewards: start with `Samples~/BasicSurvivorsGame/Content/DefaultRunFlow/run-flow.json`, `CreateDefaultTuning`, and `CreateTuning(SurvivorsPacingProfile)` before changing controller internals.
