...
Playable Unity template package for a Survivors-style horde roguelite loop. The sample boots into a top-down arena with radial enemy spawning, auto-attacks, XP gems, rarity-tinted reward cards, level-up choices, readable damage feedback, enemy hit flashes, death bursts, ranged enemy shot cues, streak reward banners, major-reward cache beacons, reward-pick banners, low-health warnings, major-threat telegraphs, miniboss and boss pressure, elite upgrade rewards, boss evolution rewards, relic rewards, class unlocks, persistent meta progression, victory, endless continuation, defeat, and restart flow.
...
Weapons auto-fire toward nearby targets. XP gems pull toward the player when close, pulse as they are attracted, kill streaks call out bonus XP, vital shards, magnet recalls, and blood-shard drops with short banners, vital shards restore health when wounded, blood shards add run bonus currency for meta progression, and magnet pickups or the debug recall make distant XP gems spin and surge inward with stronger pickup feedback.
...
- Run tuning, pacing profiles, horde pack sizes, pickup heal/currency amounts, endless major-threat intervals, reward timeout, draft rarity weights, draft rerolls, draft banishes, and skip rewards: start with `CreateDefaultTuning` and `CreateTuning(SurvivorsPacingProfile)` before changing controller internals.
