...
The scene contains a tiny bootstrap object. At runtime it binds the authored enemy, run-flow, and reward JSON assets, then creates the run-mode selector, arena, player, enemy/pickup/projectile pools, camera, top-center run timer, HUD, draft UI, relic UI, meta profile service, result summary state, endless continuation, and restart flow.
...
- Enemies, pickups, miniboss, boss, and rewards: edit `Samples~/BasicSurvivorsGame/Content/DefaultEnemies/enemies.json`, `Samples~/BasicSurvivorsGame/Content/DefaultPickups/pickups.json`, `Samples~/BasicSurvivorsGame/Content/DefaultRewards/rewards.json`, and `Samples~/BasicSurvivorsGame/Content/DefaultRunFlow/run-flow.json`. The sample scene binds those enemy, run-flow, and reward JSON assets through `BasicSurvivorsGameBootstrap` and `SurvivorsAuthoredContentDefinition`; fallback runtime defaults remain available for missing explicit content, but the playable sample uses the authored pools. Enemy JSON owns role, recycle/leash/reposition flags, marker style, and major-threat life-bar behavior; random runtime selection should stay inside these authored pools.
...
- Run tuning, pacing profiles, dash distance/cooldown/safety/shove values, orbit knockback values, horde pack sizes, enemy crowd-spacing radius/strength, ranged-shot dodge XP rewards, normal enemy leash/recycle bands, major-threat catch-up/reposition/marker distances, Gem Rush duration/bonuses, level-up pulse damage/radius, low-health Clutch Pulse damage/radius/safety, full-weapon Arsenal Surge duration/bonuses/pulse, full-passive Harmony Surge duration/bonuses/pulse, payload hazard Trap Chain threshold/window/cooldown/XP/pulse values, timed horde-rush events, clear caches, breaker-pulse damage/radius, and Breaker Surge rewards, roaming cache cadence/specials/ambushes/Wayfinder Surge rewards, Arena Trial cadence/pressure/clear rewards, waystone discovery rewards/focus/ambushes/Waystone Chain rewards, regular summoner support cadence/counts/extra alive allowance, evolution-surge damage/radius, Legend Surge thresholds/duration/bonuses/pulse, boss relic surge duration/bonuses/pulse, reward jackpot payouts, major-threat support call-in thresholds/counts, major-threat slam interval/telegraph/radius/damage, kill-streak reward/surge pacing, pickup heal/currency amounts, pickup magnet pulse intervals, Sprint XP curve and level-up draft throttle, endless major-threat intervals, Endless Surge reward knobs, reward timeout, draft rarity weights, draft rerolls, draft banishes, and skip rewards: start with `Samples~/BasicSurvivorsGame/Content/DefaultRunFlow/run-flow.json`, `CreateDefaultTuning`, and `CreateTuning(SurvivorsPacingProfile)` before changing controller internals.
...
The menu validates sample JSON and runtime catalogs for IDs, archetype references, projectile references, upgrade targets, pickup definitions, pickup-build upgrade coverage, vertical-slice weapon role coverage, enemy role coverage, enemy recycle/leash/marker lifecycle fields, boss/miniboss enemy definitions, relic effects, class IDs, starting weapons, passive atlases, weapon skill tracks, Sprint XP/draft throttle fields, and unlock requirements. The report is written to the Unity console for template developer visibility.
