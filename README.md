...
- Run tuning, pacing profiles, dash distance/cooldown/safety/shove values, horde pack sizes, enemy crowd-spacing radius/strength, timed horde-rush events and clear caches, roaming cache cadence/specials/ambushes, evolution-surge damage/radius, major-threat support call-in thresholds/counts, kill-streak reward/surge pacing, pickup heal/currency amounts, endless major-threat intervals, reward timeout, draft rarity weights, draft rerolls, draft banishes, and skip rewards: start with `CreateDefaultTuning` and `CreateTuning(SurvivorsPacingProfile)` before changing controller internals.
