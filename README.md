...
- Run tuning, pacing profiles, horde pack sizes, timed horde-rush events, kill-streak reward/surge pacing, pickup heal/currency amounts, endless major-threat intervals, reward timeout, draft rarity weights, draft rerolls, draft banishes, and skip rewards: start with `CreateDefaultTuning` and `CreateTuning(SurvivorsPacingProfile)` before changing controller internals.
