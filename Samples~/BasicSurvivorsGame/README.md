...
- The editable sample content includes `Content/DefaultRunFlow/run-flow.json` for pacing-profile reference values: spawn pressure, elite cadence, horde rush timing, roaming cache cadence, draft thresholds, rarity weights, endless intervals, boss timing, and victory timing.
...
Run `Tools > Deucarian > Templates > Survivors > Validate Content` after editing sample weapons, upgrades, enemies, pickups, rewards, relics, classes, progression tracks, run-flow pacing, or rarity weights.
